...
/**
 * docx_to_pdf.php
 * Converts a submitted DOCX file to PDF using LibreOffice (headless)
 * and streams the PDF inline so the browser can display it in an iframe.
 *
 * Usage: docx_to_pdf.php?submission_id=123
 */
session_start();
...
if (!empty($diskPath) && !file_exists($diskPath)) {
    ...
    if ($resolved && file_exists($resolved)) $diskPath = $resolved;
}
if (empty($diskPath) || !file_exists($diskPath)) {
...
// If it's already a PDF, just stream it directly
if ($ext === 'pdf') {
...
// Copy source file to temp dir (LibreOffice writes output next to the input)
$srcCopy = $tmpDir . '/' . basename($diskPath);
copy($diskPath, $srcCopy);

// Find LibreOffice binary
$loPaths = [
    // Windows — with and without .exe extension
...
foreach ($loPaths as $p) {
    // file_exists works for .exe on Windows; also try shell where/which
    if (file_exists($p)) {
        $soffice = $p;
        break;
    }
...
if (!$soffice) {
    // LibreOffice not found — clean up and return a helpful error page
    @rmdir($tmpDir);
    ...
    h3{margin:0 0 .5rem;color:#333;}p{color:#666;font-size:.9rem;margin:0;}
    </style></head><body>
    <div class="box">
      <div class="icon">📄</div>
      <h3>Preview unavailable</h3>
      <p>LibreOffice is not installed on this server.<br>
         Install LibreOffice to enable DOCX preview.</p>
    </div></body></html>';
    exit;
}

// Run conversion: soffice --headless --convert-to pdf --outdir /tmp/... file.docx
$cmd = sprintf(
...
$pdfName = pathinfo($srcCopy, PATHINFO_FILENAME) . '.pdf';
...
if ($returnCode !== 0 || !file_exists($pdfPath)) {
    ...
    .icon{font-size:3rem;margin-bottom:1rem;}
    h3{margin:0 0 .5rem;color:#c0392b;}p{color:#666;font-size:.85rem;margin:.5rem 0 0;}
    </style></head><body>
    <div class="box">
